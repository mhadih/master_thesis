...
from uer.utils.mask import mask_seq, mask_seq_edit
import numpy as np
        ...
        for _ in range(self.dup_factor):
            ...
            with open(self.corpus_path, mode="r", encoding="utf-8") as f:
                ...
                while True:
                    line = f.readline()
                    pos += 1
                    ##line__ = self.tokenizer.convert_tokens_to_ids(line.split())
                    document = self.tokenizer.convert_tokens_to_ids(line.split('\t')[0].split())

                    ### Original UER Corpus (Chinese Dataset)
...
class MlmDataLoader(DataLoader):
    def __iter__(self):
        while True:
            ...
            for ins in instances:
                if len(ins) == 3:
                    ## static masking and the length is 3
                    src.append(ins[0])
                    masked_words_num += len(ins[1])
                    ...
                    ## dynamic masking does not have "tgt" here and the length is 2
...
class Mlm_Code_Change_and_Edit_DataLoader(DataLoader):
    def __iter__(self):
        while True:
            while self._empty():
                self._fill_buf()
            ...
            for ins in instances:
                ...
                before_code, after_code, edit_actions = ins[0], ins[1], ins[2]

                def prepare_src_tgt_seg(code):
                    src_ = []
                    tgt_ = []
                    seg_ = []
                    ...
                    ## static masking and the length is 3
                    src_.append(code[0])
                    tgt_.append([0] * len(code[0]))
                    for mask in code[1]:
                        tgt_[-1][mask[0]] = mask[1]
                    seg_.append([1] * code[2][0] + [0] * (len(code[0]) - code[2][0]))
                    return src_, tgt_, seg_

                b_src, b_tgt, b_seg = prepare_src_tgt_seg(before_code)
                a_src, a_tgt, a_seg = prepare_src_tgt_seg(after_code)
                e_src, e_tgt, e_seg = prepare_src_tgt_seg(edit_actions)

                assert(b_seg == a_seg == e_seg)

                before_src.append(b_src[0])
                after_src.append(a_src[0])
                edit_src.append(e_src[0])

                before_tgt.append(b_tgt[0])
                after_tgt.append(a_tgt[0])
                edit_tgt.append(e_tgt[0])

                seg.append(b_seg[0])

            yield torch.LongTensor(before_src), \
                  torch.LongTensor(after_src), \
                  torch.LongTensor(edit_src), \
                  torch.LongTensor(before_tgt), \
                  torch.LongTensor(after_tgt), \
                torch.LongTensor(edit_tgt), \
                torch.LongTensor(seg)


class Mlm_Code_Change_and_Edit_Dataset(Dataset):
    def __init__(self, args, vocab, tokenizer):
        super(Mlm_Code_Change_and_Edit_Dataset, self).__init__(args, vocab, tokenizer)
        self.full_sentences = args.full_sentences

    def worker(self, proc_id, start, end):
        count_instances_all = 0
        print("Worker %d is building dataset ... " % proc_id)
        set_seed(self.seed)
        dataset_writer = open("dataset-tmp-" + str(proc_id) + ".pt", "wb")
        docs_buffer = []
        for _ in range(self.dup_factor):
            ...
            with open(self.corpus_path, mode="r", encoding="utf-8") as f:
                ...
                while True:
                    ...
                    line = f.readline()
                    before_version, edits, after_version = line.split('\t')
                    assert(len(before_version.split()) == len(edits.split()) == len(after_version.split()))
                    pos += 1

                    before_document = [self.vocab.get(CLS_TOKEN)] + self.tokenizer.convert_tokens_to_ids(before_version.split()) + [self.vocab.get(SEP_TOKEN)]
                    after_document = [self.vocab.get(CLS_TOKEN)] + self.tokenizer.convert_tokens_to_ids(after_version.split()) + [self.vocab.get(SEP_TOKEN)]
                    edit2id_dict = {"pad":0, "equal":1, "delete": 2, "insert":3, "replace":5, "mask":4 , "special":6, "text":7}
                    edit_ids = [6] + [edit2id_dict[e] for e in edits.split()] + [6]


                    ### Original UER Corpus (Chinese Dataset)
                    #document = [self.vocab.get(CLS_TOKEN)] + self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(line)) + [self.vocab.get(SEP_TOKEN)]
                    #document1 = [self.vocab.get(CLS_TOKEN)] + self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(line1)) + [self.vocab.get(SEP_TOKEN)]
                    if self.full_sentences:
                        if len(document) > 0:
                            docs_buffer.append(document)
                        if len(docs_buffer) == self.docs_buffer_size:
                        ...
                        if len(before_document) > 0:
                            """
                            instances = self.build_instances(before_document)
                            #instances1 = self.build_instances(after_document)
                            instances1 = self.mask_same_position_for_after_code(instances, after_document)
                            instances_edit = self.build_instances(edit_ids, flag='edit')
                            """
                            # strategy_list = ['whole_unit', 'before_masking', 'after_masking', 'actions_masking']
                            #                 # 50%               20%               20%              10%
                            # prob = random.random()
                            # if prob<0.5:
                            #     strat = 'whole_unit'
                            # elif (0.5<prob) & (prob <0.7):
                            #     strat = 'before_masking'
                            # elif  (0.7<prob) & (prob <0.9):
                            #     strat = 'after_masking'
                            # elif (prob >0.9):
                            #     strat = 'actions_masking'
                            # else:
                            #     strat = 'whole_unit'


                            # 专门用于做ablation study， 我不准备在论文里写before masking了
                            # 对于各个预训练任务完全拆开
                            strategy_list = ['ab_whole_unit', 'ab_after_masking', 'ab_actions_masking']
                                               # 34%               33%                33%
                            prob = random.random()
                            ## three pretraining tasks together (30w steps; 256 batch size; 128 max length)
                            # if prob <= 0.33:
                            #     strat = 'ab_whole_unit'
                            # elif (0.33 < prob) & (prob <= 0.66):
                            #     strat = 'ab_after_masking'
                            # elif (prob > 0.66):
                            #     strat = 'ab_actions_masking'
                            # else:
                            #     strat = 'ab_whole_unit'
                            #
                            # ## wo/ whole_unit (30w steps; 256 batch size; 128 max length)
                            # if prob <= 0.5:
                            #     strat = 'ab_after_masking'
                            # else:
                            #     strat = 'ab_actions_masking'
                            #
                            ## wo/ masking_after (30w steps; 256 batch size; 128 max length)
                            # if prob <= 0.5:
                            #     strat = 'ab_whole_unit'
                            # else:
                            #     strat = 'ab_actions_masking'
                            #
                            # ## wo/ masking_action (30w steps; 256 batch size; 128 max length)
                            if prob <= 0.5:
                                strat = 'ab_whole_unit'
                            else:
                                strat = 'ab_after_masking'
                            ...
                            #print(strat)
                            instances, instances1, instances_edit = self.choosing_masking_strategy(before_document, after_document, edit_ids, strategy=strat)
                            #print(len(instances) , len(instances1) , len(instances_edit))
                            assert(len(instances) == len(instances1) == len(instances_edit))
                            # Save instances.
                            for i_instance in range(len(instances)):
                                #single_instance = {"before": instances[i_instance], "after": instances1[i_instance], "action": instances_edit[i_instance]}
                                single_instance = (instances[i_instance], instances1[i_instance], instances_edit[i_instance])
                                count_instances_all +=1
                                pickle.dump(single_instance, dataset_writer)
                    if pos >= end:
                        break
    ...
    def build_instances(self, all_documents, flag=None):
        instances = []
        instances_num = len(all_documents) // self.seq_length

        ## If there are more than 1 sequences (the sentence is too long and is divided into sub_sequences)
        for i in range(instances_num):
            src = all_documents[i * self.seq_length: (i + 1) * self.seq_length]
            seg_pos = [len(src)]
    ...
    def empty_masking(self, all_documents, flag=None):
        ## do not mask anything, only masking the CLS token for ensuing the same format
        list_ = all_documents
        # length_ = [len(all_documents)]
        length_ = np.count_nonzero(np.array(list_))
        list_[0] = 4
        if flag == 'edit':
           ground_truths = [(0, 6)]
        else:
           ground_truths = [(0, 1)]
        empty_masking_ins = [( list_, ground_truths, length_ )]
        return empty_masking_ins

    def mask_same_position_for_after_code(self, before_ins, after_code):

        def get_masked_positions_of_before(before_data):
            if len(before_data) == 1:
                before_data = before_data[0]
            changed_positions = [p[0] for p in before_data[1]]
            return changed_positions

        instances = []
        instances_num = len(after_code) // self.seq_length

        ## multiple sequences
        for i in range(instances_num):
            src = after_code[i * self.seq_length: (i + 1) * self.seq_length]
            seg_pos = [len(src)]

            positions =  get_masked_positions_of_before(before_ins[i])
            tgt = []
            for p in positions:
                token = src[p]
                tgt.append( (p, token))
                src[p] = self.vocab.get(MASK_TOKEN)

            instance = (src, tgt, seg_pos)
            instances.append(instance)

        ## single sequence (len < max_sequence_length)
        src = after_code[instances_num * self.seq_length:]
        seg_pos = [len(src)]
        while len(src) != self.seq_length:
            src.append(self.vocab.get(PAD_TOKEN))
        positions = get_masked_positions_of_before(before_ins[instances_num])
        tgt = []
        for p in positions:
            token = src[p]
            tgt.append((p, token))
    ...
    def choosing_masking_strategy(self, before_document, after_document, edit_ids, strategy=None):

        def masking_changed_code(vocab, one_doc, another_doc, edit_act):
            before_document = one_doc
            after_document = another_doc

            changed_token_indexs = []
            for jj in range(len(edit_act)):
                id_ = edit_act[jj]
                if id_ in [0, 1, 6, 7]:
                    continue
                else:
                    changed_token_indexs.append(jj)

            # for i in range(len(before_document) - 1, -1, -1):
            #     if before_document[i] != 0:
            #         break
            # before_no_pad = before_document[:i + 1]
            # after_no_pad = after_document[:i + 1]

            random.shuffle(changed_token_indexs)
            num_to_predict_changed = max(1, int(round(len(changed_token_indexs) * 0.9)))

            before_tgt = []
            after_tgt = []

            if len(changed_token_indexs) == 0:
                changed_token_indexs.append(random.randint(1,len(one_doc)-1)) ## for before
                changed_token_indexs.append(random.randint(1, len(one_doc) - 1)) ## for after

            for index_set in changed_token_indexs:

                if len(before_tgt) < num_to_predict_changed:
                    token = before_document[index_set]
                    index = index_set
                    before_tgt.append((index, token))
                    before_document[index_set] = vocab.get(MASK_TOKEN)

                else:
                    token = after_document[index_set]
                    index = index_set
                    after_tgt.append((index, token))
                    after_document[index_set] = vocab.get(MASK_TOKEN)

            return before_document, before_tgt, after_document, after_tgt

        ## masking 15% whole unit (i.e., B+A+E)
        if strategy== 'ab_whole_unit':
            instances_before = self.build_instances(before_document)
            instances_after = self.mask_same_position_for_after_code(instances_before, after_document)

            instances_edit = []
            instances_num = len(edit_ids) // self.seq_length
            for i in range(instances_num):
                edit = edit_ids[i * self.seq_length: (i + 1) * self.seq_length]
                ins_edit = self.empty_masking(edit, flag='edit')
                instances_edit.extend(ins_edit)

            edit = edit_ids[instances_num * self.seq_length:]
            while len(edit) != self.seq_length:
                edit.append(self.vocab.get(PAD_TOKEN))
            ins_edit = self.empty_masking(edit, flag='edit')
            instances_edit.extend(ins_edit)

            return instances_before, instances_after, instances_edit

        ## masking 90% changed before_code and 10% changed after_code
        # if strategy== 'ab_before_masking':
        #     instances_before = []
        #     instances_after = []
        #     instances_num = len(before_document) // self.seq_length
        #     for i in range(instances_num):
        #         before_ = before_document[i * self.seq_length: (i + 1) * self.seq_length]
        #         after_ = after_document[i * self.seq_length: (i + 1) * self.seq_length]
        #         edit_ = edit_ids[i * self.seq_length: (i + 1) * self.seq_length]
        #         seg_pos = [len(before_)]
        #         before_masked, before_tgt, after_masked, after_tgt = masking_changed_code(self.vocab, before_, after_, edit_)
        #         before_ins = (before_masked, before_tgt, seg_pos)
        #         after_ins = (after_masked, after_tgt, seg_pos)
        #         instances_before.append(before_ins)
        #         instances_after.append(after_ins)
        #
        #     before_ = before_document[instances_num * self.seq_length:]
        #     after_ = after_document[instances_num * self.seq_length:]
        #     edit_ = edit_ids[instances_num * self.seq_length:]
        #     seg_pos = [len(before_)]
        #     while len(before_) != self.seq_length:
        #         before_.append(self.vocab.get(PAD_TOKEN))
        #         after_.append(self.vocab.get(PAD_TOKEN))
        #         edit_.append(self.vocab.get(PAD_TOKEN))
        #
        #     before_masked, before_tgt, after_masked, after_tgt = masking_changed_code(self.vocab, before_, after_, edit_)
        #
        #     before_tgt = sorted(before_tgt, key=lambda x: x[0])
        #     after_tgt = sorted(after_tgt, key=lambda x: x[0])
        #
        #     before_ins = (before_masked, before_tgt, seg_pos)
        #     after_ins = (after_masked, after_tgt, seg_pos)
        #     instances_before.append(before_ins)
        ...
        ## masking 90% changed after_code and 10% changed before_code
        if strategy== 'ab_after_masking':
            instances_before = []
            instances_after = []
            instances_edit = []
            instances_num = len(before_document) // self.seq_length
            for i in range(instances_num):
                ...
                edit_ = edit_ids[i * self.seq_length: (i + 1) * self.seq_length]
                seg_pos = [len(before_)]
                after_masked, after_tgt, before_masked, before_tgt = masking_changed_code(self.vocab, after_, before_, edit_)
                before_ins = (before_masked, before_tgt, seg_pos)
                after_ins = (after_masked, after_tgt, seg_pos)
                instances_before.append(before_ins)
                instances_after.append(after_ins)
                ins_edit = self.empty_masking(edit_, flag='edit')
                instances_edit.extend(ins_edit)
        ...
        ## masking all actions
        if strategy== 'ab_actions_masking':
            # instances_before = self.build_instances(before_document)
            # instances_after = self.mask_same_position_for_after_code(instances_before, after_document)
            # instances_before = self.empty_masking(before_document)
            # instances_after =  self.empty_masking(after_document)
            def mask_all_edit_actions (vocab, edit_ids):

                for i in range(len(edit_ids) - 1, -1, -1):
                    if edit_ids[i] != 0:
                        break
                edit_no_pad = edit_ids[:i + 1]
                edit_masked = [4]*len(edit_no_pad) + [0]* (len(edit_ids) - len(edit_no_pad))
                edit_mlm_tgt = [(i, edit_no_pad[i])  for i in range(len(edit_no_pad))]
                return edit_masked, edit_mlm_tgt
            ...
            for i in range(instances_num):
                ...
                edit =    edit_ids[i * self.seq_length: (i + 1) * self.seq_length]
                seg_pos = [len(edit)]
                edit_masked, edit_tgt = mask_all_edit_actions(self.vocab, edit)
                edit_ins = (edit_masked, edit_tgt, seg_pos)
                instances_edit.append(edit_ins)

                ins_before = self.empty_masking(before_)
                ins_after  = self.empty_masking(after_)
                instances_before.extend(ins_before)
                instances_after.extend(ins_after)
            ...
            edit_masked, edit_tgt = mask_all_edit_actions(self.vocab, edit)
            edit_tgt = sorted(edit_tgt, key=lambda x: x[0])
            edit_ins = (edit_masked, edit_tgt, seg_pos)
            instances_edit.append(edit_ins)
    ...
    def worker(self, proc_id, start, end):
        ...
        with open(self.corpus_path, mode="r", encoding="utf-8") as f:
            ...
            while True:
                ...
                add_mlm_with_lm = True
                if add_mlm_with_lm == True:
                    ## Do masking on src parts
                    src, tgt_mlm = mask_seq(self.tokenizer.convert_tokens_to_ids(document_src.split()), self.tokenizer, self.whole_word_masking, self.span_masking, self.span_geo_prob, self.span_max_length)
                else:
                    ## Only Prefix LM
                    src = self.tokenizer.convert_tokens_to_ids(document_src.split())

                tgt = self.tokenizer.convert_tokens_to_ids(document_tgt.split())
