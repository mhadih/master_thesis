...
class MlmEditTarget(nn.Module):
    ...
    def __init__(self, args, vocab_size):
        super(MlmEditTarget, self).__init__()
        ...
        if self.factorized_embedding_parameterization:
            ...
            ## For after code
            self.mlm_linear_3 = nn.Linear(args.hidden_size, args.hidden_size)
            self.mlm_linear_4 = nn.Linear(args.hidden_size, self.vocab_size)
            ## For Edit types
            self.mlm_linear_5 = nn.Linear(args.hidden_size, args.hidden_size)
            self.mlm_linear_6 = nn.Linear(args.hidden_size, 8) ## only several different types : {"pad":0, "equal":1, "delete": 2, "insert":3, "replace":5, "mask":4 , "special":6, "text":7}

        self.softmax = nn.LogSoftmax(dim=-1)

        self.criterion = nn.NLLLoss()

    def before_mlm(self, memory_bank, tgt_mlm_):
        tgt_mlm = tgt_mlm_
    ...
    def after_mlm(self, memory_bank, tgt_mlm_):
        tgt_mlm = tgt_mlm_
        # Masked language modeling (MLM) with full softmax prediction.
        output_mlm = self.act(self.mlm_linear_3(memory_bank))
        output_mlm = self.layer_norm(output_mlm)
        if self.factorized_embedding_parameterization:
            output_mlm = output_mlm.contiguous().view(-1, self.emb_size)
        else:
        ...
        output_mlm = output_mlm[tgt_mlm > 0, :]
        tgt_mlm = tgt_mlm[tgt_mlm > 0]
        output_mlm = self.mlm_linear_4(output_mlm)
        output_mlm = self.softmax(output_mlm)
        denominator = torch.tensor(output_mlm.size(0) + 1e-6)
        if output_mlm.size(0) == 0:
            correct_mlm = torch.tensor(0.0)
    ...
    def edit_mlm(self, memory_bank, tgt_mlm_):
        tgt_mlm = tgt_mlm_
        # Masked language modeling (MLM) with full softmax prediction.
        output_mlm = self.act(self.mlm_linear_5(memory_bank))
        output_mlm = self.layer_norm(output_mlm)
        if self.factorized_embedding_parameterization:
            output_mlm = output_mlm.contiguous().view(-1, self.emb_size)
        else:
        ...
        output_mlm = output_mlm[tgt_mlm > 0, :]
        tgt_mlm = tgt_mlm[tgt_mlm > 0]
        output_mlm = self.mlm_linear_6(output_mlm)
        output_mlm = self.softmax(output_mlm)
        denominator = torch.tensor(output_mlm.size(0) + 1e-6)
        if output_mlm.size(0) == 0:
            correct_mlm = torch.tensor(0.0)
    ...
    def forward(self, memory_bank, tgt, seg):
        ...
        before_tgt, after_tgt, edit_tgt = tgt
        before_loss, before_correct, before_denominator = self.before_mlm(memory_bank, before_tgt)
        after_loss, after_correct, after_denominator = self.after_mlm(memory_bank, after_tgt)
        edit_loss, edit_correct, edit_denominator = self.edit_mlm(memory_bank, edit_tgt)

        loss = before_loss + after_loss + edit_loss
        correct, denominator, loss_logit = (before_correct,after_correct, edit_correct), (before_denominator, after_denominator, edit_denominator), (before_loss.item(), after_loss.item(), edit_loss.item())
        return loss, correct, denominator, loss_logit
