...
class MlmTrainer(Trainer):
    ...
    def report_and_reset_stats(self):
        ...
        f_ =  open('./models/mlm_logs.txt', 'a')
        print( str(self.current_step) + ' steps /' + str(self.total_steps) + ' steps.' + '  Loss: '+ str(self.total_loss / self.report_steps) +  ' acc:' + str(self.total_correct / self.total_denominator) + '\n' , file=f_)
        self.total_loss = 0.0
        self.total_correct = 0.0
        self.total_denominator = 0.0

class MlmEditTrainer(Trainer):
    def __init__(self, args):
        super(MlmEditTrainer, self).__init__(args)
        self.total_before_correct = 0.0
        self.total_before_denominator = 0.0
        self.total_before_loss = 0.0
        self.total_after_correct = 0.0
        self.total_after_denominator = 0.0
        self.total_after_loss = 0.0
        self.total_edit_correct = 0.0
        self.total_edit_denominator = 0.0
        self.total_edit_loss = 0.0

    def forward_propagation(self, batch, model):
        #src, tgt, seg = batch
        before_src, after_src, edit_src, before_tgt, after_tgt, edit_tgt, seg = batch
        src = (before_src, after_src, edit_src)
        #src = before_src
        tgt = (before_tgt, after_tgt, edit_tgt)
        loss_info = model(src, tgt, seg)
        loss, correct, denominator, loss_logit = loss_info
        before_correct, after_correct, edit_correct = correct
        before_denominator, after_denominator, edit_denominator = denominator
        before_loss, after_loss, edit_loss= loss_logit

        self.total_loss += loss.item()

        self.total_before_loss += before_loss
        self.total_after_loss += after_loss
        self.total_edit_loss += edit_loss

        self.total_before_correct += before_correct.item()
        self.total_after_correct += after_correct.item()
        self.total_edit_correct += edit_correct.item()

        self.total_before_denominator+= before_denominator.item()
        self.total_after_denominator+= after_denominator.item()
        self.total_edit_denominator+= edit_denominator.item()
    ...
    def report_and_reset_stats(self):
        ...
        self.logger.info("| {:8d}/{:8d} steps"
                         "| {:8.2f} tokens/s"
                         "| Total loss {:7.2f}"
                         "| before acc: {:3.3f} | after acc: {:3.3f} | action acc: {:3.3f} | Before loss {:7.2f} | After loss {:7.2f} | Action loss {:7.2f}".format(
            self.current_step,
            self.total_steps,
            done_tokens / (time.time() - self.start_time),
            self.total_loss / self.report_steps,
            self.total_before_correct / self.total_before_denominator,
            self.total_after_correct / self.total_after_denominator,
            self.total_edit_correct / self.total_edit_denominator,
            self.total_before_loss / self.report_steps,
            self.total_after_loss / self.report_steps,
            self.total_edit_loss / self.report_steps))

        f_ = open('./models/mlm__edit_logs.txt', 'a')
        print(str(self.current_step) + ' steps /' + str(self.total_steps) + ' steps.' + '  Loss: ' + str(
            self.total_loss / self.report_steps) + ' before acc:' + str(self.total_before_correct / self.total_before_denominator) +  ' after acc:' + str(self.total_after_correct / self.total_after_denominator) + ' edit acc:' + str(self.total_edit_correct / self.total_edit_denominator) +'\n', file=f_)
        self.total_loss = 0.0
        #self.total_correct = 0.0
        #self.total_denominator = 0.0
...
str2trainer = {"bert": BertTrainer, "mlm": MlmTrainer, "lm": LmTrainer,
               ...
               "bart": BartTrainer, "prefixlm": PrefixlmTrainer, "edit":MlmEditTrainer}
